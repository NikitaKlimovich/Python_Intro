...
logging.basicConfig(level=logging.INFO, filename="source_code//log//py_log.log",filemode="w",format="%(asctime)s %(levelname)s %(message)s")
...
class ArgParserManager:
    ...
    def validate_output_format(output_format):
        if output_format not in ('json','xml'):
            logging.error("Format {output_format} doesn't supported by program, choose 'json' or 'xml'")
            return False
        return True
    ...
    def validate_arguments(self):
        if not (self.parser.r and self.parser.s and self.parser.f):
            ...
            return False
        return ArgParserManager.validate_file_path(self.parser.r) * ArgParserManager.validate_file_path(self.parser.s) * ArgParserManager.validate_output_format(self.parser.f)
